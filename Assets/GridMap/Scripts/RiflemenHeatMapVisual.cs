...
public class RiflemenHeatMapVisual : MonoBehaviour
{
    ...
    {
        MeshUtils.CreateEmptyMeshArrays(riflemenGrid.GetWidth() * riflemenGrid.GetHeight(), out Vector3[] vertices, out Vector2[] uv, out int[] triangles);
        ...
        for (int x = 0; x < riflemenGrid.GetWidth(); x++)
        {
            for (int y = 0; y < riflemenGrid.GetHeight(); y++)
            {
                ...
                UpdateHeatMapVisual(vertices, uv, triangles, index, riflemenGrid.GetWorldPosition(x, y) + quadSize * .5f, quadSize, riflemenValueUV, riflemenValueUV, color);
            }
        }
