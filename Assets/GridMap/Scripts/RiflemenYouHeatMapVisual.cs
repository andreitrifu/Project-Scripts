...
public class RiflemenYouHeat : MonoBehaviour
{
    private Grid riflemenYouGrid;
    private Mesh mesh;
    ...
    public void SetRiflemenYouGrid(Grid riflemenYouGrid)
    {
        this.riflemenYouGrid = riflemenYouGrid;
        UpdateRiflemenYouHeatMapVisual();

        riflemenYouGrid.OnGridValueChanged += RiflemenGridYou_OnGridValueChanged;
    }

    private void RiflemenGridYou_OnGridValueChanged(object sender, Grid.OnGridValueChangedEventArgs e)
    {
        updateMesh = true;
    }
    ...
    private void UpdateRiflemenYouHeatMapVisual()
    {
        int totalCells = riflemenYouGrid.GetWidth() * riflemenYouGrid.GetHeight();
        HeatMapMeshData meshData = new HeatMapMeshData(totalCells);

        Vector3 quadSize = new Vector3(1, 1) * riflemenYouGrid.GetCellSize();

        for (int x = 0; x < riflemenYouGrid.GetWidth(); x++)
        {
            for (int y = 0; y < riflemenYouGrid.GetHeight(); y++)
            {
                int index = x * riflemenYouGrid.GetHeight() + y;

                int riflemenValue = riflemenYouGrid.GetValue(x, y);
                ...
                {
                    pos = riflemenYouGrid.GetWorldPosition(x, y) + quadSize * 0.5f,
                    quadSize = quadSize,
